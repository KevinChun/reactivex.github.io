...
<p>
 ...
 대신, Observable이 무엇을 만들어내든 그것에 적당하게 반응하도록 대기하는 옵저버 형태의 관찰자를 만든다.
</p>
<p>
...
<p>
 ...
 하지만, ReactiveX에서는 많은 명령들이 병렬로 실행되고 그 결과들은 "옵저버"에 의해 임의의 순서대로 모아진다. 즉, 메서드를 <em>호출한다기 보단</em>, "Observable" 형태의 데이터를 조회하고 변환하는 메커니즘을 정의한 다음 
 Observable이 이벤트를 발생시키면 옵저버의 관찰자가 앞서 정의한 메커니즘을 실행한 후 결과를 리턴하도록, Observable을 <em>구독한다</em>고 표현하는 것이 올바르다.
</p>
<p>
 이 접근 방법을 통해, 서로 의존하지 않는 많은 코드들을 실행할 경우, 하나의 코드 블럭이 리턴될 때까지 기다리지 않고 다음 코드를 실행할 수 있기 때문에 여러 코드를 한번에 실행 시킬 수 있다. &mdash;
 결과적으로 코드 전체의 실행 시간은 그 중 실행 시간이 긴 실행 시간 만큼 밖에 걸리지 않는다.
</p>
<p>
